...
class AppHeader extends LitElement {
  ...
  getPhotoUrl(fotoPath) {
    if (!fotoPath || fotoPath === 'default.jpg') {
      return null;
    }
    ...
    if (fotoPath.startsWith('/uploads/')) {
      return `http://localhost:3000${fotoPath}`;
    }
    
    return `http://localhost:3000/uploads/avatars/${fotoPath}`;
  }
  ...
  renderUserAvatar() {
    const photoUrl = this.getPhotoUrl(this.currentUser?.foto);
    ...
    return html`
      <div class="user-avatar">
        ${photoUrl ? html`
          <img 
            src="${photoUrl}" 
            alt="${this.currentUser?.nombre || 'Usuario'}"
